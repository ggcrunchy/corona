...
public class CoronaRuntime {
	 ...
	 */
	CoronaRuntime(android.content.Context context, boolean isCoronaKit, int esVersion, boolean wantsDepthBuffer, boolean wantsStencilBuffer) {
		fLuaState = null;
		fWasDisposed = false;
		fTaskDispatcher = new CoronaRuntimeTaskDispatcher(this);
		fBaseDir = "";
		fIsCoronaKit = isCoronaKit;
		fGLView = new com.ansca.corona.graphics.opengl.CoronaGLSurfaceView(context, this, isCoronaKit, esVersion, wantsDepthBuffer, wantsStencilBuffer);
	...
	void reset(android.content.Context context) {
		fGLView = new com.ansca.corona.graphics.opengl.CoronaGLSurfaceView(context, this, true, 2,false, false);
		fController.setGLView(fGLView);
		fViewManager.setGLView(fGLView);
	}
