...
// int query_DXT_capability( void ); <- STEVE CHANGE
			...
			if( DXT_mode == SOIL_CAPABILITY_PRESENT )
			{
				...
				} else
				{
					...
					DDS_data = convert_image_to_DXT5(
							resampled, MIPwidth, MIPheight, channels, &DDS_size, NULL ); // <- STEVE CHANGE
				}
				if( DDS_data )
				{
	...
	)
{
	...
	if( tex_id )
	{
		...
		if( DXT_mode == SOIL_CAPABILITY_PRESENT )
		{
			...
			{
				/*	RGBA, use DXT5	*/
				DDS_data = convert_image_to_DXT5( NULL != img ? img : data, iwidth, iheight, channels, &DDS_size, NULL ); // <- STEVE CHANGE
			}
			if( DDS_data )
			{
				soilGlCompressedTexImage2D(
