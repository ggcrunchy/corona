	...
	)
{
	...
	} else
	{
		...
		DDS_data = convert_image_to_DXT5( data, width, height, channels, &DDS_size, NULL ); // <- STEVE CHANGE
	}
	/*	save it	*/
...
unsigned char* convert_image_to_DXT5(
		const unsigned char *const uncompressed,
		int width, int height, int channels,
		int *out_size, int *steps ) // <- STEVE CHANGE
{
	unsigned char *compressed;
	int i, j, x, y;
	...
	int index = 0, chan_step = 1;
	int has_alpha;
	int rpos, gpos, bpos, apos; // <- STEVE CHANGE
	/*	error check	*/
	*out_size = 0;
	if( (width < 1) || (height < 1) ||
		(NULL == uncompressed) ||
	...
	for( j = 0; j < height; j += 4 )
	{
		for( i = 0; i < width; i += 4 )
		{
			...
			for( y = 0; y < my; ++y )
			{
				for( x = 0; x < mx; ++x )
				{
					ublock[idx++] = uncompressed[(j+y)*width*channels+(i+x)*channels+rpos]; // <- STEVE CHANGE
					ublock[idx++] = uncompressed[(j+y)*width*channels+(i+x)*channels+gpos]; //chan_step]; <- STEVE CHANGE
					ublock[idx++] = uncompressed[(j+y)*width*channels+(i+x)*channels+bpos]; //chan_step+chan_step]; <- STEVE CHANGE
					ublock[idx++] =
						has_alpha * uncompressed[(j+y)*width*channels+(i+x)*channels+apos]; //channels-1] <- STEVE CHANGE
						+ (1-has_alpha)*255;
				}
				for( x = mx; x < 4; ++x )
				{
