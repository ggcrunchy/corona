...
/*	    v = v * player->mod->master_volume / 64.0;*/
    ...
    /* Render. */
	// STEVE CHANGE
	size = player->frames_per_tick * 2;
	if (size > player->size) {
		free(player->out);
	    player->out = (float*)mallocsafe(size * sizeof(float));
		player->size = size;
	}
	memset(player->out, 0, size * sizeof(float));
	// /STEVE CHANGE

    player->mixer->Commit(player->mixer, player->out, player->frames_per_tick, mix_divisor); // <- STEVE CHANGE

	player->get = 0; // <- STEVE CHANGE
}
    
// STEVE CHANGE
#define SLOTS(player) player->size

float* ARM_GetRenderResults(ARM_Tracker* player, int* n, int* done)
{
	float* out;
	if (player->get < 0 || !player->out || !n || !done) {
		*n = 0;
		*done = 1;
		return NULL;
	}

	if (!*n || player->get + *n > SLOTS(player))
		*n = SLOTS(player) - player->get;

	out = player->out + player->get;
	player->get += *n;

	if (player->get == SLOTS(player))
		player->get = -1;
...
int ARM_HasResults(ARM_Tracker* player)
{
	return player->get >= 0;
}
// /STEVE CHANGE
