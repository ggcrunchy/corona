...
/* Points into the result of the last render, up to *n values.
   The pointer will be advanced internally for the next call.
   If fewer than *n values are available, it will be clamped to the actual number.
   If *n is 0, get the current pointer and the number of available values, without advancing.
   If *done is true, no more results are available. */
// /STEVE CHANGE
