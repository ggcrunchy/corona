...
typedef struct SoftMixerPrivData {
 ...
 //   SoftMixerOutputProc output; <- STEVE CHANGE
    /* Output callback. */
...
static int Commit(Mixer* mixer, float* stereo_out, unsigned int frames, float divisor) // <- STEVE CHANGE
{
    // float* stereo_out; <- STEVE CHANGE
    unsigned int v;
    unsigned int frame;
    SoftMixerPrivData* data = PRIV(mixer);
    ...
    // free(stereo_out); <- STEVE CHANGE

    return 0;
}


Mixer* MXR_InitSoftMixer(float rate, /*SoftMixerOutputProc output, */void* user) // <- STEVE CHANGE
{
    Mixer* mixer;
...
//    PRIV(mixer)->output = output; <- STEVE CHANGE
