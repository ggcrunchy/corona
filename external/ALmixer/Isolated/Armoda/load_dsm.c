...
#undef fread
#define fread(a, b, c, _) ALmixer_RWread(rw_ops, a, b, c)

#undef ftell
#define ftell(_) ALmixer_RWtell(rw_ops)

#undef fseek
#define fseek(_, a, b) ALmixer_RWseek(rw_ops, a, b)

/* Reads a RIFF ID and length into the given buffers.
   Returns 0 on success, -1 on failure. */
static int ReadRIFFTag(ALmixer_RWops *rw_ops/*FILE* f*/, char* buf, unsigned int* length, unsigned int* next) // <- STEVE CHANGE
{
    uint32_t len;
...
int ARM_LoadModule_DSM(ARM_Module* mod, ALmixer_RWops* rw_ops)//const char *filename) <- STEVE CHANGE
{
    char riff_tag[5];
...
/STEVE CHANGE */
    ...
    if (ReadRIFFTag(/*f*/rw_ops, riff_tag, &riff_length, &riff_next) != 0) goto failure; // <- STEVE CHANGE
	...
	if (ReadRIFFTag(/*f*/rw_ops, riff_tag, &riff_length, &riff_next) != 0) // <- STEVE CHANGE
	    break;
