...
ShaderFactory::ShaderFactory( Display& owner, const ProgramHeader& programHeader, const char * backend ) // <- STEVE CHANGE
...
ShaderFactory::Initialize()
{
	...
	if ( ShaderBuiltin::PushDefaultShell( L, fBackend ) ) // <- STEVE CHANGE
	{
...
//	Rtt_TRACE( ( "Fragment source:\n%s\n", program->GetFragmentShaderSource() ) );
	...
	//TODO - move this into a delegate block, rather than extending this
	{
		...
		// /STEVE CHANGE
			...
			/*Program::Language */language = Program::kOpenGL_ES_2; // <- STEVE CHANGE
			...
			/*Program::Language */language = Program::kOpenGL_2_1; // <- STEVE CHANGE
		#endif
		// STEVE CHANGE
		}
		// /STEVE CHANGE
